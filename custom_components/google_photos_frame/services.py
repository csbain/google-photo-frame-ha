...
def _get_entries(hass: HomeAssistant) -> list[GooglePhotosFrameConfigEntry]:
    """Get all loaded config entries for this domain."""
    return list(hass.config_entries.async_loaded_entries(DOMAIN))


def _photo_to_dict(photo: Any, hass: HomeAssistant | None = None, entry_id: str | None = None) -> dict[str, Any]:
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    def _get_all_coordinators() -> list[tuple[str, GooglePhotosFrameCoordinator]]:
        """Get all coordinators with their entry IDs."""
        return [(entry.entry_id, entry.runtime_data) for entry in _get_entries(hass)]

    async def handle_sync_now(_call: ServiceCall) -> None:
        """Handle sync_now service - refresh all coordinators."""
        for _entry_id, coord in _get_all_coordinators():
            await coord.async_request_refresh()
        _LOGGER.info("Google Photos Frame sync triggered for %d entries", len(_get_entries(hass)))

    async def handle_get_current_photo(call: ServiceCall) -> dict[str, Any]:
        ...
        # Find the actual entry_id if not provided
        actual_entry_id = entry_id
        if not actual_entry_id:
            for eid, coord in _get_all_coordinators():
    ...
    async def handle_get_photos(call: ServiceCall) -> dict[str, Any]:
    ...
    async def handle_next_photo(call: ServiceCall) -> dict[str, Any]:
    ...
    async def handle_previous_photo(call: ServiceCall) -> dict[str, Any]:
    ...
    async def handle_set_photo(call: ServiceCall) -> dict[str, Any]:
    ...
    async def handle_clear_cache(call: ServiceCall) -> dict[str, Any]:
